...
- **Pretext:** main measures letter-spaced text with `ctx.letterSpacing`, so ligature pairs under letter spacing come
  out too narrow in Safari. The rebuild sets `ctx.letterSpacing` too, measures the merged pairs it can find apart
  (the evidence below) and reports such lines under its `letter-spacing-ligatures` gap: WebKit's Canvas has no way to
  measure the text without ligatures.
