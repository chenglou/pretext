...
2. The "revert until a hyphen fits" loop. LineBuilder compares without the 1/64 epsilon and never tests index 0, falling back to the first opportunity (`IL/InlineLineBuilder.cpp:1868-1886`). The simple builder compares with the epsilon and accepts index 0 unconditionally (`IL/TextOnlySimpleLineBuilder.cpp:459-479`).
   - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: probe 6 is inconclusive. A and B give identical line starts at every 1/64px width from 16.78 to 70.39px (both `aabb‐` at 40.90625), but probe 6's conditions can't all hold in 16px Arial (w(cc) = 16 > H = 5.328125). Source reading from that run: LineBuilder already adds the hyphen width, with the epsilon, to a candidate ending in U+00AD (`IL/InlineLineBuilder.cpp:1154-1165, 1172-1183`), and `processInlineContent` then picks WrapWithHyphen (`IL/InlineContentBreaker.cpp:114-120`). So this loop (`IL/InlineLineBuilder.cpp:1860-1887`) looks reachable only when the hyphen wasn't counted, for example a non-text item after the soft-hyphen item inside the candidate (`aa&shy;<span></span>bb`). Not yet probed.
...
Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: "the same style" is coarser than a fresh build. The key's context maps `Preserve` and `BreakSpaces` to one value (`L/text/TextBreakingPositionContext.h:43-56`), storage width isn't in the key, and only the fresh path gives break-spaces one item per space (`:972-978`). In one process, break-spaces `abc      xyz` laid out after the same text in pre-wrap keeps all six spaces on line 1; alone it wraps per space. 16-bit keep-all breaks carry over to later 8-bit text with the same content. The cache is filled when a block's line layout is destroyed (`W/layout/integration/inline/LayoutIntegrationLineLayout.cpp:210-221`), so identical text and style can break differently depending on what the WebContent process laid out and tore down before.
...
- **break-spaces**: nothing hangs and nothing is trimmed.
  ...
  - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: holds in a fresh process (`abc ` / 5 spaces / `def`). After a pre-wrap layout of the same text in the same process, the break-position cache gives one six-space item, and all six spaces stay on line 1 (§3.1 note).
...
6. **Builder difference: hyphen revert epsilon.** A: `<div>aa&shy;bb&shy;cc&shy;dd</div>`; B: the same with `<span>a</span>` in place of the first "a". Choose `W` so that "dd" overflows after "aabbcc", `H > T(W) + 1/64 - w(aabbcc)`, and `T(W) - w(aabb) < H <= T(W) + 1/64 - w(aabb)`. Expect A: line 1 "aabb‐". Expect B: line 1 "aa‐" (LineBuilder's loop tests "bb" without the epsilon and falls back to the first opportunity). (§2 difference 2)
   - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: inconclusive. The four conditions can't all hold in 16px Arial (w(cc) = 16 > H = 5.328125). A and B agree at every 1/64px width from 16.78 to 70.39px, both `aabb‐` at 40.90625. See the §2 difference 2 note.
...
11. **pre-wrap spaces hang; break-spaces wrap.** `"abc      def"` (6 spaces), `W = T(M('abc') + M(' '))`. pre-wrap: 2 lines, "abc      " / "def", and the first root inline box width excludes the unconditionally hanging spaces. break-spaces: the spaces spread over lines; no line starts with the white space that ended a previous word. nowrap/pre: 1 line.
    - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: confirmed in a fresh process (pre-wrap `abc      ` / `def` with the hanging spaces excluded from alignment; break-spaces `abc ` / 5 spaces / `def`; nowrap and pre 1 line). A run that had laid out the same text in pre-wrap first kept all six spaces on line 1 under break-spaces (§3.1 note).
...
13. **White-space-only node.** `<div>\r</div>` and `<div> \t\n\f</div>` produce height 0 (no renderer), while `<div></div>` produces one line (`R/updating/RenderTreeUpdater.cpp:536-590`; `WTF/ASCIICType.h:154-157`).
    - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16 (refuted for the empty div): `<div></div>` is 0px tall, because an empty block has no line box (CRITIC W7). textContent `"\r"` and `" \t\n\f"` give 0px and `"\v"` 20px. The HTML parser turns a literal CR into LF.
...
22. **Rest measured fresh when the style differs on the first line.** `::first-line { font-size: 16px }` against a 16px paragraph (equal fonts): the carry is kept. With `::first-line { letter-spacing: 1px }` the rest is measured fresh (`IL/AbstractLineBuilder.cpp:84-91`).
    - Measured in webkit-host (system WebKit 22625.1.29.11.27; installed Safari 27.0 not run) on 2026-09-16: inconclusive. `::first-line { font-size: 16px }` keeps the carry ([0, 11, 22] at 112.75, 113.5 and 114.75px). With `letter-spacing: 1px` the two probe encodings disagree: one gave [0, 10, 21, 32] at all three widths, the other [0, 11, 22, 33], [0, 11, 22, 33] and [0, 11, 22]. Neither the fresh nor the carry model predicts the 114.75px result; settling it needs a `breakWord` port with the first-line style.
