...
- Probes in **webkit-host**: a background WKWebView app on the system WebKit 22625.1.29.11.27, the build installed
  Safari 27.0 runs (`specs/probes-safari.md`).
- Installed Safari 27.0, once the maintainer approved it (16:22 to 16:45): every development and held-out lab case in
  both orders, and both WebKit probe files. Its observations equal webkit-host's on all 89 probes, on 133 of 140
  cross-check probes (the other 7 differ from process history) and on every lab case apart from 28 held-out
  observations, each history-dependent in one browser. Its predictions from Canvas equal the host's on every case
  (`lab/WEBKIT-HOST.md` "Combined runs of the final sets", REPORT §2.2, `specs/probes-safari.md` "Installed Safari
  27.0").
...
Each WebKit finding from a browser says whether it is **verified in installed Safari**, **contradicted** by it, or
still **host only**. None is contradicted.
...
1. **`wrap-06c1e0111950efed`** (family `keep-all`, origin "safari ideographic punctuation keep-all boundary").
   ...
   - **Installed Safari:** verified, from main's suite rows, and webkit-text H15 and webkit-canvas H14 in installed
     Safari.
...
2. **`wrap-4faaad4b08f18c01`** (family `reported/#210-#211`).
   ...
   - **Installed Safari:** verified, from main's suite rows.
...
| Safari 27 | CR takes its own glyph's advance (0 in Arial). FF, VT and other Cc take `.notdef` (12px in 16px Arial). | Measure U+0001 in place of FF, VT and other Cc, in the same string. Canvas and DOM differences are both 8 (webkit-canvas H10). |
...
- **Installed Safari:** the Safari row is verified (cross-cutting 2, webkit-lines H14, webkit-text H11, H12,
  webkit-canvas H10). Main's WebKit failure counts above are webkit-host rows. Installed Safari derives the same lines
  as them on every suite-sample case compared that isn't history-dependent (REPORT §2.3).
...
| Safari 27 | `W(size)`, bit-exact (probes-safari cross-cutting 1, verified in installed Safari), as PLATFORM_BUGS already says |
...
- **Installed Safari:** the WebKit row is verified (webkit-lines H1, H2, cross-cutting 6).
...
- **Safari 27 break-rule changes** (WebKit 7625 against 7624, which is Safari 26.5.2; specs/webkit-text.md §15):
  ...
  - **Installed Safari:** verified (webkit-text H13, H4, H5; section 1).
...
- **WebKit page history.** Identical text and style can break differently depending on what the same WebContent
  ...
  - 13 of main's 2,986 old Safari rows differed from host rows only because of page order (lab/WEBKIT-HOST.md
    "Order within a page").
  - **Installed Safari:** verified. On the combined final case files, 73 of 25,180 development cases and 124 of 15,205
    held-out cases are history-dependent in installed Safari, against 73 and 132 in webkit-host on the same files. With
    the same document history, the two browsers differ on 28 held-out observations, each history-dependent in one of
    them (REPORT §2.2).
...
- **String storage from JSON.** In JavaScriptCore, `Response.json()` of a body with any raw non-Latin-1 character gives
  ...
  - **Installed Safari:** verified. The cross-check payload probes, and webkit-lines H9, webkit-text H15 and
    webkit-canvas H14 from JS literals, observe what webkit-host did.
...
- **Range geometry.** Quirks main's extractor may meet (lab/README.md "Range geometry, per browser"; lab/VALIDATION.md
  ...
  - **Installed Safari:** the WebKit and Safari items are verified. They come from installed Safari's smoke rows, and
    its final lab rows derive the same lines as webkit-host's.
...
- WebKit's one-float32-step difference between its shortcut and full measuring paths at fractional sizes
  (probes-safari correction 5) is too small to report. It holds in installed Safari too.
