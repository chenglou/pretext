...
import type { GeckoCharacter, GeckoTextFrame } from '../../src/engines/gecko/geometry.ts'
import type { CssFont, Paragraph } from '../../src/model.ts'
import { NO_BOX_EDGE, UNKNOWN_FONT_FACTS } from '../../src/model.ts'
...
const WIDTH = 500
const observe = (p: Paragraph, layout: GeckoLayout, measure: CanvasMeasure): ExpectedObservation => observeGecko(p, WIDTH, layout, measure)
...
function paragraph(texts: string[], direction: 'ltr' | 'rtl' = 'ltr'): Paragraph {
  return {
    ...
    letterSpacing: 0, wordSpacing: 0, lineHeight: 20, whiteSpace: 'normal', wordBreak: 'normal', overflowWrap: 'normal',
...
function line(frames: GeckoTextFrame[], start: number, end: number): GeckoLine {
  return {
    start, end, fragments: [], hasLineBox: true, joinsNextLine: false, slot: { left: 0, right: 0 }, indented: false, align: 'start',
...
function layout(lines: GeckoLine[]): GeckoLayout {
  return {
    ...
    lines, belowFloats: [], measure: { contexts: 0, calls: 0, memoHits: 0 }, gaps: [],
  }
}
...
describe('Range rects over Gecko frames', () => {
  test('aaaa bbbb broken after the space: the trimmed space reports width 0 at the box end', () => {
    ...
    const o = observe(paragraph(['aaaa bbbb']), l, noMeasure)
  ...
  test('a cluster reports its advance on its last code point (F2)', () => {
    ...
    const o = observe(paragraph(['é']), l, noMeasure)
  ...
  test('a −90 au space alone in its frame grows the box by the floored delta (c-79e5272a2644d9b8, F8)', () => {
    ...
    expect(values(observe(paragraph([' ']), l, noMeasure).codePoints[0]!.rects)).toEqual([[0, 0, 1.5]])
  })
  ...
  test('points count from the right edge of an RTL frame', () => {
    ...
    const o = observe(paragraph(['אב'], 'rtl'), l, noMeasure)
  ...
  test('a used soft hyphen reports the hyphen advance on the SHY (F11)', () => {
    ...
    const o = observe(paragraph(['aa­']), l, noMeasure)
  ...
  test('a lone VT with 1px letter spacing reports 60 au (c-92b6963ae4344985)', () => {
    ...
    expect(values(observe(paragraph(['\v']), l, noMeasure).codePoints[0]!.rects)).toEqual([[0, 0, 1]])
  })

  test('a node without a frame reports nothing (F12)', () => {
    const l = layout([line([frame(1, 1, 4, 0, 1728, [ch(576), ch(576), ch(576)])], 0, 4)])
    const o = observe(paragraph([' ', 'abc']), l, noMeasure)
  ...
  test('code point edges collapsed at a line start report width 0 at the measured start', () => {
    ...
    const o = observe(paragraph(['  a']), l, noMeasure)
    expect(values(o.codePoints[0]!.rects)).toEqual([[0, 0, 0]])
  ...
  test('a position the layout marks a stand-in is limited by in-word-prefix, with everything measured from it', () => {
    ...
    const o = observe(paragraph(['ab cd']), words, noMeasure)
    ...
    const s = observe(paragraph(['abcd', 'e']), split, noMeasure)
  ...
  test('a frame whose Canvas size is not the DOM size: every value is limited by font-size-quantization', () => {
    ...
    const o = observe(paragraph(['ab', 'c']), l, noMeasure)
  ...
  test('a sum over text a ranged paragraph gap names is limited by that gap', () => {
    ...
    const o = observe(paragraph(['abcd']), l, noMeasure)
    ...
    expect(observe(paragraph(['abcd']), l, noMeasure).codePoints.map(c => gaps(c.rects[0]!))).toEqual([['predicted', 'predicted'], ['predicted', 'predicted'], ['predicted', 'predicted'], ['predicted', 'predicted']])
  })
  ...
  test('an edge 2^16 device px from the origin is limited by float32-precision (probe gecko-port F6)', () => {
    ...
    const o = observe(paragraph(['ab']), l, noMeasure)
    const gaps = (r: ExpectedRect) => [r.x, r.width].map(v => v.state === 'limited' ? v.gap : v.state)
    ...
    expect(gaps(observe(paragraph(['ab']), at60(32768 * 60 - 576), noMeasure).codePoints[1]!.rects[0]!)).toEqual(['float32-precision', 'float32-precision'])
    expect(gaps(observe(paragraph(['ab']), at60(32768 * 60 - 1152), noMeasure).codePoints[1]!.rects[0]!)).toEqual(['predicted', 'float32-precision'])
  })
})
