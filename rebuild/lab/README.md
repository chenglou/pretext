...
It writes `<out>/<browser>-rows.ndjson`, one row per case, and `<out>/<browser>-run.json` with totals, page
contexts, the environment and errors. It exits nonzero when anything goes wrong: invalid cases, a launch or page
...
fonts. Cases are grouped by context, stable in order of first appearance, and every context change reloads the page.
...
5. Calls `predict(c, { browser, dpr })`. When it returns lines, it calls `paint(c, prediction, host)`. If that returns
   ...
   records the height, the Range rects of every text node inside it, and the horizontal extent of those rects.
...
A line can hold only zero-width content, such as a ZWSP, a joiner or a soft hyphen alone at a narrow width. Zero-area
rects of code points other than LF establish such a line where they sit half a line height or more from every
positive rect. Nearer, they place nothing: Safari's extra zero-width rect on the previous line, collapsed spaces at a
line edge. In a 5,000-case Chrome run, all 464 paragraphs whose height disagreed with the lines of positive rects had
...
`break-spaces`. The other space separators at a line end (U+3000, U+2000–U+200A and so on) are excluded, and that
...
- `widths`: scored only when breaks pass. The observed width is the line's horizontal extent. When nothing on the
  ...
  the visible code points' own rects. The observed and predicted values are both snapped to the browser's layout
  grid (1/64px for Chrome and Safari, 1/60px for Firefox, rounding half up), and the metric passes only on equality.
  The summary keeps a histogram of predicted minus observed, in grid units.
...
`--per-case` writes each case's four metrics.

## Range geometry, per browser

These findings from the smoke runs shape the rules above:

- Chrome 153: Range rects carry float glyph positions, not layout units. Arial `h` at 16px is 8.8984375px, so rect
  values fall on a 1/128px grid, and line extents can land on a half unit (190.3046875 snaps to 190.3125).
  `rectGrid` in the summary counts off-grid code point rect values. A letter after a selected soft hyphen (in RTL
