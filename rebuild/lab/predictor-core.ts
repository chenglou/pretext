...
import { fillLine, firstLine, linePieces, paragraphGaps, prepare } from '../src/index.ts'
import {
...
// cache of shaped words could show (specs/blink-canvas.md §1.7).
function layoutParagraph(paragraph: LayoutParagraph, env: Environment, width: number, insets: readonly LineSlot[] = [], otherWidthsFirst: readonly number[] = []): Pick<LayoutPrediction, 'layout' | 'painter'> {
  countCanvasWork()
...
export function makePredictor(factsFor: FactsFor, otherWidthFactors: readonly number[] = []): Predictor {
  return {
    predict(c, env) {
      ...
      return { paragraph, width, ...layoutParagraph(paragraph, e, width, c.inline?.lineSlots ?? [], otherWidthFactors.map(factor => width * factor)) }
    },
