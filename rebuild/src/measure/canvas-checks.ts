...
//
// Each port gives its own list (engines/<engine>/checks.ts), read from its recipes, naming only what a recipe sets to
// something other than the attribute's default. `wordSpacing = '0px'`, `fontKerning = 'auto'` and `textRendering = 'auto'`
// are defaults: a Canvas without the attribute measures the same.
type ContextAttribute = 'lang' | 'letterSpacing' | 'wordSpacing' | 'textRendering' | 'direction'

export type CanvasNeeds = {
...
export function missingCanvasSupport(needs: CanvasNeeds): string[] {
