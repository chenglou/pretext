...
against `prepare()` run as its two halves, and against the three forms of E, and flags a row where they differ.

Read A against E in the headline passes, not in the timed rows. While a row is timed the page holds the row's kept
paragraphs for B (about 11 live canvases a message in Chrome where every message made its own), and a variant that makes
