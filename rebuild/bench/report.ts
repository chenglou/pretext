...
function renderChat(out: string[], context: ContextReport, chat: ChatReport): void {
  ...
  if (chat.headlines.length > 0) {
    ...
    for (let h = 0; h < chat.headlines.length; h++) {
      ...
      out.push(`| | rebuild scratch, count, one list of contexts a pass | ${headline.rebuildKeepingMs.map(formatMs).join(', ')} | ${formatMs(keeping)} | ${formatMs(keeping / headline.messages)} | ${ratio(keeping, main)} | ${count(headline.lines.rebuildKeeping)} |`)
    ...
    if (chat.headlineResizes.length > 0) {
      ...
      for (let h = 0; h < chat.headlineResizes.length; h++) {
        ...
        out.push(`| | rebuild, count, one list of contexts | ${formatMs(resize.rebuildKeepingPrepareAndFillMs)} | ${formatMs(resize.rebuildKeepingResizeMs)} | ${formatMs(resize.rebuildKeepingResizeMs / layouts)} |`)
  ...
  for (let p = 0; p < chat.phases.length; p++) {
    ...
    out.push('')
    out.push(`### Phases, ${phases.set}${phases.keeping ? ', one list of contexts a pass' : ''}: where the rebuild's from-scratch time goes (${count(phases.messages)} messages, median of ${phases.passes} instrumented passes)`)
    out.push('')
...
function chatCells(report: BenchReport, setId: ChatSetId): Map<string, string> {
  ...
  // E: one list of contexts for the set's messages, started inside the timing (rebuild/src/index.ts prepare).
  timed('E. from scratch with one list of contexts for the set, per message', 'rebuild scratch, count, page keeps both')
  ...
  if (scratch !== undefined && keepsBoth !== undefined) cells.set('E against A: one list of contexts over a list a message', ratio(keepsBoth.stats.medianMs, scratch.stats.medianMs))
  if (headline !== undefined) {
    const keeping = median(headline.rebuildKeepingMs)
    cells.set(`E. headline: ${count(headline.messages)} messages from scratch with one list of contexts a pass`, `${formatMs(keeping)} (${formatMs(keeping / headline.messages)} per message; passes ${headline.rebuildKeepingMs.map(formatMs).join(', ')})`)
  }
  timed('E. the font checks\' contexts alone come from the list, per message', 'rebuild scratch, count, page keeps checks')
  timed('E. the engine\'s contexts alone come from the list, per message', 'rebuild scratch, count, page keeps contexts')
