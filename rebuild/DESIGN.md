...
- A wrapped line start beside a space that the port's width tests call safe, on a line whose fit test (its end, or the
  ...
  `in-word-prefix` over the line and the content its decision measured: HarfBuzz can flag such a start with no width
  signature, and Blink then reshapes it and corrects the space by 0 or −1 LayoutUnits (shaping_line_breaker.cc:309-324),
  which moves the line's end. `edgeGap` reported the same condition at a start inside a word already, at the start
...
- A wrapped line start taken from a stand-in position reports its reach, under the name the position rests on
  (`limits.ts` `positionLimit`), over the same range where the line's fit lies within what the position can be off by
...
in pinned Chrome 153 against fresh natives, the scorer calls all 84 covered, 70 by one of the four new conditions. Where
they fire: on the tier cases without facts in 5,582 of 69,224 cases (8.1%; `one-unit-fit` 4,661, `stand-in-start-reach`
807, `common-window-side` 208, `white-space-window-side` 0), most of them in the rule families, whose widths are derived
from the native lines, and with the lab's facts in 6,132; on the real-text sets, see TAKEOVER.md.
