...
| `pairKerning` | Blink and WebKit no; Firefox yes, per offset (since correctness round 5, 2026-09-19) | Canvas shows the pair's total adjustment. In Blink and WebKit which glyph carries it never reaches widths or bounding boxes, and no browser ships per-glyph metrics. Putting a combining mark between the pair doesn't separate the two kinds either. Blink was probed again in the round (probe blink-cr5 K): it keeps 16.16 advances and rounds no glyph, so no total moves with the placement. Gecko rounds each glyph's advance to app units, so the three placements (all on the first glyph, half on each, all on the second) can give totals one app unit apart, and widths at the size times 2^k give the fractions. Where the fact isn't given, the Gecko port asks per offset between two kerned glyphs, never as a fact of the declaration (`engines/gecko/advance.ts`; DESIGN.md §4.4). The adjustment is what the unit's two measured sides show crossing the offset, which the pair measured alone must equal, or be within 2 app units of for halves. The pair's own total tells a placement where that one gives the adjustment and the other two don't. Where it doesn't tell, 16 probe pairs asked once per Canvas context of a prepared paragraph may, for a pair Canvas shows the probe letters' face draws. That path rests on one inference: a face places all its Latin pairs one way. A pair that isn't told stays a stand-in under `in-word-prefix`. Firefox's 304 lost cases above were counted before this. |
