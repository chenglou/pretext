...
**Landed (round 4c, 2026-09-18).** The five rules are in the engines, each read again in the pinned source, with unit tests and registry rows: B1 and W1 came with round 4b (`blink/output/box-fragment-edges`, `webkit/output/first-and-last-display-box`), G1, G2 and B2 with round 4c (`gecko/measure/tab-width-containing-block` restated, `gecko/lines/preserved-newline-ends-line-in-br`, `blink/output/end-offset-for-justify`). On this document's set, run again with the merged library: Firefox passes every prediction metric on the main set and the slots family with 0 of 93,852 and 0 of 6,672 predicted values differing; Chrome keeps 1 differing value, the observation limit of `c-a37545c096e939be`; webkit-host has 0. Nothing moved on the tier sets' statuses. The set is a tier set now (`rich-prewrap`, lab README "Test tiers"), and what is left of §3 C and D is in `rebuild/tests/known-tail.json`. The numbers are in specs/{gecko,blink,webkit}-RESULTS.md, "Round 4c". The two webkit-host widths one float32 step off (`c-7ab87463d0a47df0`, `c-ea6d4a54b03e0a22`) fail the same way alone in a fresh process and in installed Safari, so `page-history`, which fires on their line, covers them by position only.
