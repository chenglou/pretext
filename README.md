...
Use-case 2 APIs:
```ts
prepareWithSegments(text: string, font: string, options?: { whiteSpace?: 'normal' | 'pre-wrap', wordBreak?: 'normal' | 'keep-all', letterSpacing?: number, editable?: boolean }): PreparedTextWithSegments // same as `prepare()`, but returns a richer structure for manual line layout needs
...
prepareEdit(previous: PreparedTextWithSegments, text: string): PreparedTextWithSegments // also accepts and returns a PreparedText. For text that changes a bit at a time, like a textarea's value or a streamed reply: pass a state prepared with `{ editable: true }` and the whole new text. Returns what prepare() or prepareWithSegments() of `text` would, with the previous state's font and options, but re-measures only around the change. The result is editable too, and the previous state isn't mutated. Editable states keep a copy of their text
```
