...
async function smokeJavaScriptEsm(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "if (typeof pretext.createIncrementalPreparer !== 'function') throw new Error('incremental owner export missing')",
...
async function smokeTypeScript(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "import { createIncrementalPreparer } from '@chenglou/pretext'",
      "const owner = createIncrementalPreparer('16px Arial', { whiteSpace: 'pre-wrap' })",
      "layout(owner.append('hello'), 100, 20).height satisfies number",
      "owner.replace('replacement').segments satisfies string[]",
      "owner.clear()",
      '',
    ].join('\n'),
  )
