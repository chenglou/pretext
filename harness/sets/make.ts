...
//
//   bun harness/sets/make.ts write                           # sample, reports and oracles
...
//   bun harness/sets/make.ts cut <set>                        # the cut cases, into harness/cases/<set>.ndjson
...
function templatesOf(set: string): Template[] {
  switch (set) {
    case 'catalog': return catalogTemplates().catalog
    case 'facts': return factTemplates()
    case 'rich': return richTemplates(catalogTemplates().rich)
...
switch (command) {
  case 'write': {
    ...
    const sets: Array<[string, Case[]]> = [['reports', reportCases()], ['oracles', oracleCases()], ['sample', sample.cases]]
  ...
  case 'bisect':
    ...
    break
  case 'cut':
    printSizes(set!, writeCases(join(CASES, `${set}.ndjson`), cut(set!, templatesOf(set!))))
    break
  case 'sizes': {
